...
/* Question number 3 */
class SymptomPolyuria : Fragment() {
    ...
    override fun onCreateView(
        ...
        *  */
        (activity as DetectionActivity).initSelectedOptions(
            key = Constanta.DiabetesSympthoms.Polyuria.name,
        ...
        (activity as DetectionActivity).initNavigationQuestion(
            ...
            prevFragment = DetectionActivity.fragmentGender,
            nextQuestion = binding.nextQuestion,
            nextFragment = DetectionActivity.fragmentPolydipsia,
            nextValidation = (activity as DetectionActivity).checkQuestionIsFilled(Constanta.DiabetesSympthoms.Polyuria.name)
        )

        /* init progress filled -> percentage current step of 15 question */
        binding.progressBar.progress = 13
    ...
    private fun updateUI() {
        ...
        (activity as DetectionActivity).switchFragment(DetectionActivity.fragmentPolydipsia)
    }
