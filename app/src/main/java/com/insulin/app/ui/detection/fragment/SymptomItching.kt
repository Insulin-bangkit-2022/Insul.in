...
class SymptomItching : Fragment() {
    ...
    override fun onCreateView(
        ...
        *  */
        (activity as DetectionActivity).initSelectedOptions(
            key = Constanta.DiabetesSympthoms.Itching.name,
        ...
        (activity as DetectionActivity).initNavigationQuestion(
            ...
            prevFragment = DetectionActivity.fragmentGenitalThrus,
            nextQuestion = binding.nextQuestion,
            nextFragment = DetectionActivity.fragmentIrritability,
            nextValidation = (activity as DetectionActivity).checkQuestionIsFilled(Constanta.DiabetesSympthoms.Itching.name)
        )

        /* init progress filled -> percentage current step of 15 question */
        binding.progressBar.progress = 54
    ...
    private fun updateUI() {
        ...
        (activity as DetectionActivity).switchFragment(DetectionActivity.fragmentIrritability)
    }
