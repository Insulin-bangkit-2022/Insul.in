...
/* Question number 14 */
class SymptomAlopecia : Fragment() {
    ...
    override fun onCreateView(
        ...
        *  */
        (activity as DetectionActivity).initSelectedOptions(
            key = Constanta.DiabetesSympthoms.Alopecia.name,
        ...
        (activity as DetectionActivity).initNavigationQuestion(
            ...
            prevFragment = DetectionActivity.fragmentMuscleStiffness,
            nextQuestion = binding.nextQuestion,
            nextFragment = DetectionActivity.fragmentObesity,
            nextValidation = (activity as DetectionActivity).checkQuestionIsFilled(Constanta.DiabetesSympthoms.Alopecia.name)
        )

        /* init progress filled -> percentage current step of 15 question */
        binding.progressBar.progress = 86
