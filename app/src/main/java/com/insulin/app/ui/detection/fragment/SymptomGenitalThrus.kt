...
/* Question number 8 */
class SymptomGenitalThrus : Fragment() {
    ...
    override fun onCreateView(
        ...
        *  */
        (activity as DetectionActivity).initSelectedOptions(
            key = Constanta.DiabetesSympthoms.GenitalThrus.name,
        ...
        (activity as DetectionActivity).initNavigationQuestion(
            ...
            prevFragment = DetectionActivity.fragmentPolyphagia,
            nextQuestion = binding.nextQuestion,
            nextFragment = DetectionActivity.fragmentItching,
            nextValidation = (activity as DetectionActivity).checkQuestionIsFilled(Constanta.DiabetesSympthoms.GenitalThrus.name)
        )

        /* init progress filled -> percentage current step of 15 question */
        binding.progressBar.progress = 47
    ...
    private fun updateUI() {
        ...
        (activity as DetectionActivity).switchFragment(DetectionActivity.fragmentItching)
    }
